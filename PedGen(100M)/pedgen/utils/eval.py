...
# 包含多个计算与姿势评估和度量相关的函数，主要用于计算预测结果与真实值之间的误差

class AverageMeter(object):# 计算并存储某些数值的平均值和当前值
    """Computes and stores the average and current value"""

    def __init__(self):
        self.reset()

    def reset(self):# 重置所有统计数据
        self.avg = 0
        self.sum = 0
        self.count = 0

    def update(self, vals):# 更新当前值、总和和计数，并计算新的平均值
...
def compute_pose_metrics(pred, gt):
    ...
    # dist_init预测和真实值的初始距离，用于评估初始姿势的准确性
    dist_init = torch.linalg.norm(pred_init - gt_init, dim=2)  # [50, t_pred]
    # we can reuse 'dist' to optimize metrics calculation

    #分别在整个时间序列和最后一帧上计算的误差
    made, _ = dist.mean(dim=1).min(dim=0)# 平均绝对误差
    mfde, _ = dist[:, -1].min(dim=0)# 最终绝对误差
    made = made.mean()
    mfde = mfde.mean()
    #计算预测轨迹与真实轨迹之间的误差
    aade = dist.mean(dim=1).mean(dim=0)# 平均欧式距离误差
    afde = dist[:, -1].mean(dim=0)# 最终欧式距离误差
    aade = aade.mean()
    afde = afde.mean()
...
def compute_traj_metrics(pred_trans, gt_trans, pred_rot, gt_rot):
    ...
    maoe_init, _ = rot_dist.min(dim=0)# 最小角度误差
    aaoe_init = rot_dist.mean(dim=0)# 平均角度误差
...
#计算与地面相关的度量，用来评估模型在步态预测中与地面接触的情况
def compute_ground_metrics(pred, ground_map, init_transl, threshold=0.05):
    ...
    mfe = mfe.mean()# 地面接触误差，评估预测步态是否接触到地面
    mfpr = mfpr.mean()# 接触渗透率，计算步态渗透或浮动的比例，超过阈值则视为渗透

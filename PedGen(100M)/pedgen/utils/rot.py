...
# 6D旋转表示，将6个不规范的数值转换成一个标准的3×3旋转矩阵
def rotation_6d_to_matrix_numpy(d6: np.ndarray) -> np.ndarray:
    ...
    a1, a2 = d6[..., :3], d6[..., 3:]# a1,a2是d6的前,后3个元素
    a1_norms = np.linalg.norm(a1, axis=-1, keepdims=True) #计算a1向量的范数
    b1 = a1 / a1_norms #将a1向量归一化
    b2 = a2 - (b1 * a2).sum(-1, keepdim=True) * b1 #计算与b1正交的向量
    b2_norms = np.linalg.norm(b2, axis=-1, keepdims=True) 
    b2 = b2 / b2_norms
    b3 = np.cross(b1, b2) #计算b1和b2的叉乘
    return np.stack((b1, b2, b3), axis=-2) #返回由3个正交单位向量（b1、b2、b3）组成的3x3矩阵

...
class PedGenModel(LightningModule):
    ...
    def __init__(
        ...
        self.noise_steps = noise_steps#把传入的保存成成员变量
    ...
    def inpaint_gt(self, x0, full_motion):#作用：在ddim采样阶段把生成结果拉回到已知约束上（要改！！！！！！）
        x0[:, 0, :3] = full_motion[:, 0, :3]#固定第0帧位移为GT的第0帧
        if self.use_goal:
            gt_goal = torch.sum(full_motion[:, :, :3], dim=1)#把时间维求和，等于GT整段轨迹的总位移
            pred_goal = torch.sum(x0[:, :, :3], dim=1)#当前生成结果的总位移
            goal_scale = gt_goal / (pred_goal + 1e-9)  # B, 3   #把整个生成的局部位移序列按比例缩放，使其总和对齐到GT goal
            x0[:, :, :3] = x0[:, :, :3] * goal_scale.unsqueeze(1)#保证生成轨迹的终点位移与GT一致

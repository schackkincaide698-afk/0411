...
class CityWalkersDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict:
        ...
        tt = lambda x: torch.from_numpy(x).float()#把NumPy数组转换为Tensor，神经网络才能处理
        data_dict = {}
        ...
        if self.mode == "train":#在训练阶段，动作去噪依赖给定的真实轨迹，根本不看RGB图像，直接给模型喂全0张量

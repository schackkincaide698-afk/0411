...
class Predictor(nn.Module):
    ...
    def forward(self, batch: Dict) -> Dict[str, torch.Tensor]:
        ...
        gt_goal_rel_seq_mask = batch.get("gt_goal_rel_seq_mask", None)
        scheduled_sampling_prob = float(batch.get("scheduled_sampling_prob", 0.0))
        if gt_goal_rel_seq is not None:
            gt_goal_rel_seq = gt_goal_rel_seq.to(scene_points.device)
        if gt_goal_rel_seq_mask is not None:
            gt_goal_rel_seq_mask = gt_goal_rel_seq_mask.to(scene_points.device)
        ...
        for step_idx in range(self.num_plan_steps):
            ...
            if step_idx < self.num_plan_steps - 1:
                next_input_pos = next_pred_pos
                # Teacher Forcing with mask-safe fallback:
                # only use GT when the segment GT is valid, otherwise fallback to autoregressive prediction.
                if self.training and gt_goal_rel_seq is not None and gt_goal_rel_seq_mask is not None:
                    gt_rel = gt_goal_rel_seq[:, step_idx, :]
                    next_gt_pos = feedback_history_list[-1] + gt_rel
                    use_gt_rand = (
                        torch.rand(gt_goal_rel_seq_mask.shape[0], device=scene_points.device) <
                        scheduled_sampling_prob
                    )
                    use_gt_valid = gt_goal_rel_seq_mask[:, step_idx] > 0.5
                    use_gt = (use_gt_rand & use_gt_valid).unsqueeze(-1)
                    next_input_pos = torch.where(use_gt, next_gt_pos, next_pred_pos)
                feedback_history_list.append(next_input_pos)

        pred_goal_rel_seq = torch.stack(pred_goal_rel_seq, dim=1)

        output = {"pred_goal_rel_seq": pred_goal_rel_seq}
        if "gt_traj_150" in batch:
            gt_traj_150 = batch["gt_traj_150"].to(scene_points.device)
            gt_traj_150_mask = batch.get("gt_traj_150_mask", None)
            if gt_traj_150_mask is not None:
                gt_traj_150_mask = gt_traj_150_mask.to(scene_points.device)
            ...
            if gt_traj_150_mask is not None:
                ...
                loss_full_traj = F.l1_loss(pred_traj_150, gt_traj_150)
        else:
            loss_full_traj = torch.tensor(0.0, device=scene_points.device)

        loss_env = self.compute_walkability_loss(pred_goal_rel_seq, gt_init_pos, batch)
        output["loss_full_traj"] = loss_full_traj
        output["loss_env"] = loss_env
    ...
    def compute_walkability_loss(self, pred_goal_rel_seq: torch.Tensor,
                                 gt_init_pos: torch.Tensor,
                                 batch: Dict) -> torch.Tensor:
        if "scene_tokens" not in batch:
            return torch.tensor(0.0, device=pred_goal_rel_seq.device)
        scene_points = batch["scene_tokens"].to(pred_goal_rel_seq.device)
        ...
        num_interp = 20
        interp = torch.linspace(0.0, 1.0, num_interp, device=pred_goal_rel_seq.device).view(1, 1, num_interp, 1)
        ...
        for b in range(scene_points.shape[0]):
            ...
            if len(valid_sp) < 1:
                ...
                grid_size = grid_size.to(pred_goal_rel_seq.device)
                x_min, x_max, _, _, z_min, z_max = grid_size.tolist()
                bound_penalty = (
                    torch.relu(x_min - traj_xz[b, :, 0]) +
                    torch.relu(traj_xz[b, :, 0] - x_max) +
                    torch.relu(z_min - traj_xz[b, :, 1]) +
                    torch.relu(traj_xz[b, :, 1] - z_max)
                )
                losses.append(bound_penalty.mean())
                continue
            valid_xz = valid_sp[:, [0, 2]]
            ...
            if nearby_xz.shape[0] == 0:
                min_bounds = valid_xz.min(dim=0)[0]
                max_bounds = valid_xz.max(dim=0)[0]
                penalty_min = torch.relu(min_bounds - traj_xz[b])
                penalty_max = torch.relu(traj_xz[b] - max_bounds)
                losses.append((penalty_min + penalty_max).mean())
                continue
            ...
            losses.append(torch.relu(min_dist - MARGIN).mean())
        return torch.stack(losses).mean() if losses else torch.tensor(0.0, device=pred_goal_rel_seq.device)
...
class PedGenModel(LightningModule):
    ...
    def forward(self, batch: Dict) -> Dict:
        ...
        if self.training and "gt_goal_rel_seq_mask" in batch:
            current_epoch = getattr(self, "current_epoch", 0)
            max_epochs = getattr(getattr(self, "trainer", None), "max_epochs", 1) or 1
            epoch_ratio = float(current_epoch) / float(max_epochs)
            if epoch_ratio < 0.3:
                predictor_batch["scheduled_sampling_prob"] = 1.0
            elif epoch_ratio < 0.8:
                predictor_batch["scheduled_sampling_prob"] = (0.8 - epoch_ratio) / 0.5
            else:
                predictor_batch["scheduled_sampling_prob"] = 0.0
        predictor_dict = self.predictor(predictor_batch)
        full_motion = self.get_full_motion(batch)#得到GT动作
        gt_goal_rel = batch.get("gt_goal_rel", None)
        if gt_goal_rel is None:
            raise RuntimeError("gt_goal_rel is required for GT-conditioned diffuser training")
        gt_goal_rel = gt_goal_rel.to(self.device)
        gt_init_pos = batch["gt_init_pos"].to(self.device)
        gt_new_img = OccupancyGridBuilder(batch, self.device).build(
            gt_init_pos,
            gt_goal_rel,
            is_sequence=False,
        )
        cond_embed = self.get_condition(batch, goal_rel=gt_goal_rel, new_img=gt_new_img)#最终给扩散模型的条件
    ...
    #把各种条件融合成 cond_embed
    def get_condition(self, batch, goal_rel: torch.Tensor, new_img: torch.Tensor):
        B = batch['img'].shape[0]#取 batch size
        cond_embed = self.cond_embed.unsqueeze(0).repeat(B, 1)

        if self.use_goal:
            cond_embed = cond_embed + self.goal_embed(goal_rel)
        if self.use_beta:
    ...
    def eval_step(self, batch: Dict) -> Dict:
        predictor_dict = self.predictor(batch)
        pred_goal_rel = predictor_dict["pred_goal_rel_seq"][:, 0, :]
        pred_new_img = OccupancyGridBuilder(batch, self.device).build(
            predictor_dict["gt_init_pos"],
            pred_goal_rel,
            is_sequence=False,
        )
        cond_embed = self.get_condition(batch, goal_rel=pred_goal_rel, new_img=pred_new_img)
    ...
    def predict_step(self, batch: Dict) -> Dict:
        ...
        for i in range(num_segments):#把4段动作合并
            ...
            seg_predictor_dict["pred_new_img"] = OccupancyGridBuilder(batch, self.device).build(
                current_init_pos,
                planned_goal_rel[i],
                is_sequence=False,
            )
            cond_embed = self.get_condition(
                batch,
                goal_rel=planned_goal_rel[i],
                new_img=seg_predictor_dict["pred_new_img"],
            )
            seg_samples = self.sample(
                batch["img"].shape[0],
                cond_embed,
                self.num_sample,
                target_goal_rel=planned_goal_rel[i],
                hand_shake=False,
            )
        ...
        current_samples = current_samples[..., :150, :]
